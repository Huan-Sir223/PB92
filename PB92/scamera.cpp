#include "scamera.h"
#include "ui_scamera.h"
#include <qdebug.h>

scamera::scamera(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::scamera)
{
    ui->setupUi(this);
    ...
    connect(this, &scamera::updateImage, ui->widget, &PlayImage::updateImage);
}
       ...
       // if(ui->comboBox->count() <= 0) return;
        ...
        if(ret)
        {
           ...
           // ui->but_open->setText("关闭摄像头");
            QtConcurrent::run(this, &scamera::readImage);   // 在线程中读取
        }
    }
    else
    {
...
QImage scamera::MatToQImage(const Mat &mat)
{
   // qDebug()<<"mat.type()";
    switch (mat.type())
    {
